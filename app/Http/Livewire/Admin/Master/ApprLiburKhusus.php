...
class ApprLiburKhusus extends Component
{
    public function render()
    {
        return view('livewire.admin.master.appr-libur-khusus', [
    ...
    public function reqNew() {
        ...
        ReqLiburKhusus::create($libur_data);
    ...
    public function reqChange($id) {
        $libur_data = LiburKhusus::where('id', $id)->get();
        ...
        if(!empty($this->departemen_id)) {
            $new_data['list_perubahan'] = $new_data['list_perubahan'] . '<li><span>Nama Libur</span>' . $libur_data[0]->departemen_id . '</li>';
        };
        ...
        if(!empty($this->departemen_id)) {
            $new_data['list_perubahan'] = $new_data['list_perubahan'] . '<li><span>Nama Libur</span>' . $this->departemen_id . '</li>';
            $new_data['departemen_id'] = $this->departemen_id;
        } else {
            $new_data['departemen_id'] = $libur_data[0]->departemen_id;
        };
        if(!empty($this->nama_libur)) {
        ...
        if(!empty($this->nama_libur) || !empty($this->mulai) || !empty($this->sampai) || !empty($this->pengumuman)) {
            ReqLiburKhusus::create($new_data);
    ...
    public function reqDestroy($id) {
        ...
        $req_data['departemen_id'] = $libur_data[0]->departemen_id;
        ...
        ReqLiburKhusus::create($req_data);

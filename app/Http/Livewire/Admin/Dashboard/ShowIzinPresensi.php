...
class ShowIzinPresensi extends Component
{
    public function render()
    {
        ...
        if(empty(Auth::user()->departemen_id)) {
            $izin = IzinPresensi::where('perusahaan_id', Auth::user()->instansi->id)->get();
        } else {
            $izin = IzinPresensi::where('departemen_id', Auth::user()->departemen_id)->get();
        };

        return view('livewire.admin.dashboard.show-izin-presensi', [
            'izins' => $izin
        ]);
    }
}

<div wire:poll class="data-template">
    <div class="req-card-container">
        ...
        @foreach ($allAdmin as $admin)
            @foreach ($admin->reqIzinPresensi as $req)
                <div class="req-card">
                    <div class="req-head">
                    ...
                    <div class="form-form">
                        ...
                        @else
                            ...
                            <div class="form-row" style="margin-bottom: 8px">
                                <div class="form-field">
                                    <label>Mulai</label>
                                    <input type="text" value="{{ \Carbon\Carbon::parse($req->mulai)->isoFormat('dddd, D MMMM Y') }}" disabled> 
                                </div>
                                <div class="form-field">
                                    <label>Sampai</label>
                                    <input type="text" value="{{ \Carbon\Carbon::parse($req->sampai)->isoFormat('dddd, D MMMM Y') }}" disabled> 
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-field">
                                    <label>Keterangan</label>
                                    <input type="text" value="{{ $req->keterangan }}" disabled> 
                                </div>
                            </div>
                            @if ($req->jenis_pengajuan === 'Penambahan')
                                <a href="/storage/{{ $req->lampiran }}" style="display: block" target="_blank">
                                    <button class="new">Lihat Lampiran</button>
                                </a>
                            @endif
                        @endif
                    </div>
